...
void ZkClient::Start()
{
	...
	*/
    ...
    //是异步的连接过程，ip+port，全局回调，超时时间
    m_zhandle = zookeeper_init(connstr.c_str(), global_watcher, 30000, nullptr, nullptr, 0);
    //返回并不是表示连接是否成功，而是表示句柄开辟是否成功，此时是否连接已经没有意义
    if (nullptr == m_zhandle) 
    {
        std::cout << "zookeeper_init error!" << std::endl;
        exit(EXIT_FAILURE);
    }
    //当连接好以后，会在global_watcher中进行回调

    sem_t sem;
    sem_init(&sem, 0, 0);
    zoo_set_context(m_zhandle, &sem);

    sem_wait(&sem);//等待信号量资源
    std::cout << "zookeeper_init success!" << std::endl;
}

//state表示节点的状态，是永久性节点还是临时性节点，state = 0表示永久性节点
